    ...
    def __init__(
        ...
        try:
            ...
            if project_name not in project_list:
                ...
                logger.debug(f"Project '{project_name}' found in project list")
            
            self.project_id = [
                project["id"] for project in response.json()["data"]["content"] if project["name"] == project_name
            ][0]            
            # Update user data with project ID
        ...
        if tracer_type == "agentic" or tracer_type.startswith("agentic/") or tracer_type == "langchain" or tracer_type == "google-adk":
            ...
            instrumentors = []
            logger.debug(f"Initializing instrumentors for tracer type: {tracer_type}")
