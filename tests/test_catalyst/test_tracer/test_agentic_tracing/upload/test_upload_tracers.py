...
# Get the path to the module we want to test for coverage purposes
UPLOAD_MODULE_PATH = os.path.abspath(os.path.join(
    os.path.dirname(__file__),
    '../../../../../../ragaai_catalyst/tracers/agentic_tracing/upload/upload_agentic_traces.py'
))

# Mock UploadAgenticTraces class for testing
class MockUploadAgenticTraces:
    def __init__(self, 
                 json_file_path,
                 project_name,
                 project_id,
    ...
    def _get_dataset_spans(self):
        ...
        # First check if this span already exists to avoid duplicates
            if not any(s["spanHash"] == span["hash_id"] for s in datasetSpans):
                datasetSpans.append({
                    "spanId": span["id"],
                    "spanName": span["name"],
            ...
            for child in span["data"]["children"]:
                ...
                if any(s["spanHash"] == child["hash_id"] for s in datasetSpans):
                    continue
                    
                if child["type"] == "agent":
                    # Only process agent children recursively, don't add them here
                    self._get_agent_dataset_spans(child, datasetSpans)
                else:
                    # Add non-agent children directly
                    datasetSpans.append({
                        "spanId": child["id"],
                        "spanName": child["name"],
...
def uploader_instance():
    with tempfile.NamedTemporaryFile(suffix=".json") as temp_file:
        ...
        uploader = MockUploadAgenticTraces(
...
class TestUploadAgenticTraces:
    ...
    @patch.object(MockUploadAgenticTraces, '_get_presigned_url')
    @patch.object(MockUploadAgenticTraces, '_put_presigned_url')
    @patch.object(MockUploadAgenticTraces, 'insert_traces')
    ...
    @patch.object(MockUploadAgenticTraces, '_get_presigned_url', side_effect=Exception("Test error"))
