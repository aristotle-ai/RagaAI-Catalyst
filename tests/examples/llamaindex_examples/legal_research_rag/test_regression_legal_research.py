        ...
        for span in openai_spans:
            if "attributes" in span and "output.value" in span["attributes"]:
                ...
                try:
                    ...
                    print(f"Invalid JSON format in output.value: {span['attributes']['output.value']}")
                    output_json = None  # or some other default handling
                
                if output_json and "llm_output" in output_json and "token_usage" in output_json["llm_output"]:
