...
def masking_function(value):
    """
    Returns redacted values using regex for symptoms like fever, headache, fatigue.
    Replaces matched symptom terms with <redacted_symptom>.
    """
    symptoms = ['fever', 'headache', 'fatigue']
    pattern = re.compile(r'\b(' + '|'.join(symptoms) + r')\b', re.IGNORECASE)
    anonymized_result = pattern.sub('<redacted_symptom>', value)
